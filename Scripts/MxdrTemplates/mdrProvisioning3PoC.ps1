...
Clear-Host
Write-Log -Msg "Start processing PowerShell script - v1.2_000"
Write-Host
...
if ($isOt) {
    ...
    while($confirmationOtSp -ne 'y') {
        while ($null -eq $ot) {
            if ($parameterCount -gt $compareValue) {
                try { $ot = $( $paramsObject | Select-Object -ExpandProperty "OtServicePrincipal" -ErrorAction Stop ) }
                catch {
                    $ErrorMessage = $_.Exception.Message
                    $compareValue = 100
                }
                ...
                Write-Host
                $ot = Read-Host 'Enter a name for the OT Service principal to be created '
            }
        }
        while ($ot -and $confirmationOtSp -ne 'y' -and $confirmationOtSp -ne 'n') {
            Write-Host
            $confirmationOtSp = Read-Host "Please confirm you want to use $ot as the name for the OT Service principal [Y/N] "
        }
        if ($confirmationOtSp -eq 'y') {
            try { $RirSpInfo = Get-AzureADApplication -All $true -ErrorAction SilentlyContinue | Where-Object { $_.DisplayName -eq $ot } }
            catch {
                $ErrorMessage = $_.Exception.Message
                Write-Log -Sev 3 -Line (__LINE__) -Msg "There was a problem validating the RIR Service principal."
                Write-Log -Sev 3 -Line (__LINE__) -Msg $ErrorMessage
            ...
            if ($RirSpInfo) {
                if ($RirSpInfo.Length -gt 1) {
                    Write-Log -Sev 3 -Line (__LINE__) -Msg "Multiple Service principal found with name : $ot"
                    Write-Log -Sev 3 -Line (__LINE__) -Msg "Please correct and execute the scipt again."
                    Exit
                }
                $confirmationOtSp = $null
                ...
                while ($ot -and $confirmationOtSp -ne 'y' -and $confirmationOtSp -ne 'n') {
                    ...
                    Write-Log -Sev 2 -Line (__LINE__) -Msg "A Service principal with the same name has been found in the tenant."
                    Write-Host
                    $confirmationOtSp = Read-Host "Do you want to use this Service principal : $ot ? [Y/N] "
                    Write-Host
                }
                if ($confirmationOtSp -eq 'y') {
                    Write-Log -Sev 1 -Line (__LINE__) -Msg "RIR Service principal", $RirSpInfo.DisplayName, "( App ID:", $RirSpInfo.AppId, ")"
...
#---------------------------------------------------------------------------------------------------------------------------------------------------------------
Write-Log -Sev 1 -Line (__LINE__) -Msg "Building permissions object for MDE API"
$responseAtpPermission1 = New-Object -TypeName "Microsoft.Open.AzureAD.Model.ResourceAccess" -ArgumentList "93489bf5-0fbc-4f2d-b901-33f2fe08ff05","Role" #AdvancedQuery.Read.All
$responseAtpPermission2 = New-Object -TypeName "Microsoft.Open.AzureAD.Model.ResourceAccess" -ArgumentList "7e4e1300-e1b9-4102-88ba-f0cb6e6d5974","Role" #Machine.Isolate
$responseAtpPermission3 = New-Object -TypeName "Microsoft.Open.AzureAD.Model.ResourceAccess" -ArgumentList "ea8291d3-4b9a-44b5-bc3a-6cea3026dc79","Role" #Machine.Read.All
$responseAtpPermission4 = New-Object -TypeName "Microsoft.Open.AzureAD.Model.ResourceAccess" -ArgumentList "a86d9824-b2b6-45f8-b042-16bc4922ed4e","Role" #Machine.Scan
...
$responseMgPermission1 = New-Object -TypeName "Microsoft.Open.AzureAD.Model.ResourceAccess" -ArgumentList "741f803b-c850-494e-b5df-cde7c675a1ca","Role" #User.ReadWrite.All
$responseMgPermission2 = New-Object -TypeName "Microsoft.Open.AzureAD.Model.ResourceAccess" -ArgumentList "bf394140-e372-4bf9-a898-299cfc7564e5","Role" #SecurityEvents.Read.All
$responseMgPermission3 = New-Object -TypeName "Microsoft.Open.AzureAD.Model.ResourceAccess" -ArgumentList "c529cfca-c91b-489c-af2b-d92990b66ce6","Role" #User.ManageIdentities.All
$responseMgPermission4 = New-Object -TypeName "Microsoft.Open.AzureAD.Model.ResourceAccess" -ArgumentList "19dbc75e-c2e2-444c-a770-ec69d8559fc7","Role" #Directory.ReadWrite.All
...
#########################################################################
# OT service principal section
#########################################################################
if ($isOt) { 

    Clear-Host
    Write-Log -Msg "Difenda OT service principal section"
    Write-Host
    Write-Log -Sev 1 -Line (__LINE__) -Msg "Using OT service principal name:", $ot
    Write-Log -Sev 1 -Line (__LINE__) -Msg "Creating permissions object for Difenda RIR service principal"
    ...
    #--------------------------------------------
    Write-Log -Sev 1 -Line (__LINE__) -Msg "Building permissions object for Microsoft Graph API"
    $otMsgPermission1 = New-Object -TypeName "Microsoft.Open.AzureAD.Model.ResourceAccess" -ArgumentList "40f97065-369a-49f4-947c-6a255697ae91","Role" # MailboxSettings.Read
    $otMsgPermission2 = New-Object -TypeName "Microsoft.Open.AzureAD.Model.ResourceAccess" -ArgumentList "df021288-bdef-4463-88db-98f22de89214","Role" # User.Read.All
    $otMsgPermission3 = New-Object -TypeName "Microsoft.Open.AzureAD.Model.ResourceAccess" -ArgumentList "483bed4a-2ad3-4361-a73b-c83ccdbdc53c","Role" # RoleManagement.Read.Directory
    $otMsgPermission4 = New-Object -TypeName "Microsoft.Open.AzureAD.Model.ResourceAccess" -ArgumentList "693c5e45-0940-467d-9b8a-1022fb9d42ef","Role" # Mail.ReadBasic.All
    $otMsgPermission5 = New-Object -TypeName "Microsoft.Open.AzureAD.Model.ResourceAccess" -ArgumentList "7438b122-aefc-4978-80ed-43db9fcc7715","Role" # Device.Read.All
    $otMsgPermission6 = New-Object -TypeName "Microsoft.Open.AzureAD.Model.ResourceAccess" -ArgumentList "5b567255-7703-4780-807c-7be8301ae99b","Role" # Group.Read.All
    $otMsgPermission7 = New-Object -TypeName "Microsoft.Open.AzureAD.Model.ResourceAccess" -ArgumentList "98830695-27a2-44f7-8c18-0c3ebc9698f6","Role" # GroupMember.Read.All
    $otMsgPermission8 = New-Object -TypeName "Microsoft.Open.AzureAD.Model.ResourceAccess" -ArgumentList "bf394140-e372-4bf9-a898-299cfc7564e5","Role" # SecurityEvents.Read.All
    $otMsgPermission9 = New-Object -TypeName "Microsoft.Open.AzureAD.Model.ResourceAccess" -ArgumentList "7ab1d382-f21e-4acd-a863-ba3e13f7da61","Role" # Directory.Read.All
    $otMsgPermission10 = New-Object -TypeName "Microsoft.Open.AzureAD.Model.ResourceAccess" -ArgumentList "c7fbd983-d9aa-4fa7-84b8-17382c103bc4","Role" # RoleManagement.Read.All
    $otMsgPermission11 = New-Object -TypeName "Microsoft.Open.AzureAD.Model.ResourceAccess" -ArgumentList "5e1e9171-754d-478c-812c-f1755a9a4c2d","Role" # AuditLogsQuery.Read.All
    $otMsgPermission12 = New-Object -TypeName "Microsoft.Open.AzureAD.Model.ResourceAccess" -ArgumentList "b0afded3-3588-46d8-8b3d-9842eff778da","Role" # AuditLog.Read.All
    $otMsgPermission13 = New-Object -TypeName "Microsoft.Open.AzureAD.Model.ResourceAccess" -ArgumentList "21792b6c-c986-4ffc-85de-df9da54b52fa","Role" # ThreatIndicators.ReadWrite.OwnedBy

    Write-Log -Sev 1 -Line (__LINE__) -Msg "Creating Microsoft Graph permissions assignment"
    $OtMsg = New-Object -TypeName "Microsoft.Open.AzureAD.Model.RequiredResourceAccess"
    $OtMsg.ResourceAppId = '00000003-0000-0000-c000-000000000000'
    $OtMsg.ResourceAccess = $otMsgPermission1, $otMsgPermission2, $otMsgPermission3, $otMsgPermission4, $otMsgPermission5, $otMsgPermission6, $otMsgPermission7, $otMsgPermission8, $otMsgPermission9, $otMsgPermission10, $otMsgPermission11, $otMsgPermission12, $otMsgPermission13
    ...
    if ($newRir) {
        Write-Log -Sev 1 -Line (__LINE__) -Msg "Assigning API permissions ..."
        try {
            Set-AzureADApplication -ObjectId $newRir.ObjectId -RequiredResourceAccess $OtMsg, $RirMtp, $RirAtp
        }
        catch {
            $ErrorMessage = $_.Exception.Message
            Write-Log -Sev 3 -Line (__LINE__) -Msg "Failed assigning API permissions to RIR service principal"
